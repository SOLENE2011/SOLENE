...
<table width = 80% border=0 cellpadding = "0" cellspacing = "1">
	...
	<td style = "font-family: Tahoma; font-size: 8pt; font-weight: bold;"
		width = 130 height=24 bgcolor="f7f7f7">운송자(등기) 번호 </td>
	...
	<tr>
		...
		<select name = "status">
			<option value = "0" <%if(order.getORDER_STATUS()==0){%> selected
				<%}%>>대기중</option>
			<option value = "1" <%if(order.getORDER_STATUS()==0){%> selected
				<%}%>>발송 준비</option>
			<option value = "2" <%if(order.getORDER_STATUS()==0){%> selected
				<%}%>>발송 완료</option>
			
			<option value = "3" <%if(order.getORDER_STATUS()==0){%> selected
				<%}%>>배송중</option>
	
			<option value = "4" <%if(order.getORDER_STATUS()==0){%> selected
				<%}%>>배송 완료</option>
			<option value = "5" <%if(order.getORDER_STATUS()==0){%> selected
				<%}%>>주문 취소</option>
		</select>
		</td>
	</tr>
	<tr><td style = "background-color: #F0F0F0; height: 1px;" colspan=6></tr>
	<tr><td style = "background-color: #F0F0F0; heighr: 1px;" colspan=6></tr>
	<tr>	
