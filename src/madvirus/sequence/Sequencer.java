...
public class Sequencer {
	...
	public synchronized static int nextId(Connection conn, String tableName) throws SQLException {
		...
		try {
			pstmtSelect = conn.prepareStatement("select MESSAGE_ID from PKID_SEQUENCES where TABLE_NAME = ?");
			...
			if(rsSelect.next()) {
				...
				pstmtUpdate = conn.prepareStatement(
						"update PKID_SEQUENCES set MESSAGE_ID = ? " +
						"where TABLE_NAME = ?");
				...
				return id;
					...
					pstmtUpdate = conn.prepareStatement(
							"insert into PKID_SEQUENCES values(?, ?)");
